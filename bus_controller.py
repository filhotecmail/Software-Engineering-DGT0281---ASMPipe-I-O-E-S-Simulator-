...
class BusController:
    ...
    def request_bus(
        self, requester_id: int, priority: BusPriority, data_size: int
    ) -> bool:
    ...
    def get_bus_status(self) -> Dict[str, any]:
        ...
        with self.lock:
            utilization = (
                len(self.active_transfers) / self.max_concurrent_transfers * 100
            )
            ...
            return {
                ...
                "success_rate": (
                    (self.granted_requests / self.total_requests * 100)
...
class BusArbitrator:
    ...
    def priority_based_arbitration(
        self, requests: List[BusRequest]
    ) -> Optional[BusRequest]:
...
def main():
    ...
    print(
        f"Máximo de transferências simultâneas: {bus_controller.max_concurrent_transfers}"
    )
