...
class DMAChannel:
    ...
    def configure(
        self, source: int, destination: int, size: int, transfer_type: int
    ) -> bool:
...
class DMAController:
    ...
    def performance_test(self) -> Dict[str, float]:
        ...
        for i in range(10):
            ...
            if (
                self.setup_channel(
                    channel_id, 0x1000 + i * 1024, 0x2000 + i * 1024, 1024
                )
                == 0
            ):
        ...
        return {
            ...
            "transfers_per_second": (
                self.transfers_completed / (end_time - start_time)
                if end_time > start_time
                else 0
            ),
            "bytes_per_second": (
                self.bytes_transferred / (end_time - start_time)
                if end_time > start_time
                else 0
            ),
        }
